...
const BATCH_SIZE = 500; // TEST BATCH ONLY
...
async function main() {
  ...
  // In "test" mode, we only want to process a small subset (the first BATCH_SIZE records).
  // Then chunk that subset (usually just 1 chunk if BATCH_SIZE is e.g. 2).
  const testSubset = allRows.slice(0, BATCH_SIZE);
  const totalChunks = Math.ceil(testSubset.length / BATCH_SIZE);
  console.log(`Test subset length: ${testSubset.length}`);
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    const end = Math.min(start + BATCH_SIZE, testSubset.length);
    const chunkRows = testSubset.slice(start, end);
